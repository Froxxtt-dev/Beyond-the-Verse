...
async function generateBatch(section, count = 8) {
  ...
  const raw = await groqChat(cfg.system, userPrompt, { temperature: 0.95, maxTokens: 900 });
...
async function generateDetail(section, teaser) {
  ...
  const raw = await groqChat(cfg.system, userPrompt, { temperature: 0.8, maxTokens: 1200 });
  const parsed = parseJsonLoose(raw);
